...
def main_open(main_open_id, opens, user_info):
    ...
    # 좋아요 DB 정보 가져오기
    sql = "SELECT * FROM OpenLiked WHERE show_id IN ('%s', '%s', '%s', '%s')" %(main_open_id[0],main_open_id[1],
                                                                                main_open_id[2],main_open_id[3])
    conn = mysql.connect()
    cursor = conn.cursor()
    cursor.execute(sql)
...
def main_hot(main_hot_id, hots, user_info):
    ...
    # 좋아요 DB 정보 가져오기
    sql = "SELECT * FROM HotLiked WHERE show_id IN ('%s', '%s', '%s', '%s')" %(main_hot_id[0],main_hot_id[1],
                                                                               main_hot_id[2],main_hot_id[3])
    conn = mysql.connect()
    cursor = conn.cursor()
    cursor.execute(sql)
    all_likes_tmp = cursor.fetchall()
